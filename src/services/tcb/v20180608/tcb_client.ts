...
创建成功后，可通过 [DescribeAIModels](https://cloud.tencent.com/document/product/876/131318) 接口查询分组信息，并在云开发 AI+ 功能中使用所配置的模型。
  ...
  async CreateAIModel(
    ...
    cb?: (error: string, rep: CreateAIModelResponse) => void
  ): Promise<CreateAIModelResponse> {
...
注意：本接口为只读 dry-run 操作，不落库、不创建任何资源，仅返回各项检查的详细结果。本接口通过不代表 CreateHTTPServiceRoute 必然成功（例如证书运行时状态、并发抢占等仍需创建时最终判定），但本接口不通过则 CreateHTTPServiceRoute 必然不通过。
     */
  async VerifyHTTPServiceRoute(
    req: VerifyHTTPServiceRouteRequest,
    cb?: (error: string, rep: VerifyHTTPServiceRouteResponse) => void
  ): Promise<VerifyHTTPServiceRouteResponse> {
...
解绑之后，会导致云存储不可用，请谨慎操作。
     */
  async UnbindStorageSource(
    req: UnbindStorageSourceRequest,
    cb?: (error: string, rep: UnbindStorageSourceResponse) => void
  ): Promise<UnbindStorageSourceResponse> {
   ...
   */
  async ListPGUserMigrations(
    req: ListPGUserMigrationsRequest,
    cb?: (error: string, rep: ListPGUserMigrationsResponse) => void
  ): Promise<ListPGUserMigrationsResponse> {
...
每个环境仅允许绑定 1 个外部云存储源。
     */
  async BindStorageSource(
    req: BindStorageSourceRequest,
    cb?: (error: string, rep: BindStorageSourceResponse) => void
  ): Promise<BindStorageSourceResponse> {
...
安全域名被删除之后，可能会引起跨域问题，请谨慎操作。
  ...
  async ModifyHTTPServiceRoute(
    req: ModifyHTTPServiceRouteRequest,
    cb?: (error: string, rep: ModifyHTTPServiceRouteResponse) => void
  ): Promise<ModifyHTTPServiceRouteResponse> {
...
本接口仅更新存储源绑定关系，不会迁移您的数据。
     */
  async ModifyStorageSource(
    req: ModifyStorageSourceRequest,
    cb?: (error: string, rep: ModifyStorageSourceResponse) => void
  ): Promise<ModifyStorageSourceResponse> {
...
该接口跟 [DescribeTables](https://cloud.tencent.com/document/api/876/127962) 接口功能一致，后续该接口可能会下线，请使用 [DescribeTable](https://cloud.tencent.com/document/api/876/127962)接口。
  ...
  async DescribePGUserMigration(
    req: DescribePGUserMigrationRequest,
    cb?: (error: string, rep: DescribePGUserMigrationResponse) => void
  ): Promise<DescribePGUserMigrationResponse> {
...
创建流程为先调用[DescribeVmSpec](https://cloud.tencent.com/document/product/876/129360)获取可购买的规格，同时调用[DescribeBlueprints](https://cloud.tencent.com/document/product/1207/47689)拉取镜像列表，选中一个规格和一个镜像后，调用[InquireVmPrice](https://cloud.tencent.com/document/product/876/129759)询价，如果价格可接受，调用此接口创建实例
  ...
  async DeleteHTTPServiceRoute(
    req: DeleteHTTPServiceRouteRequest,
    cb?: (error: string, rep: DeleteHTTPServiceRouteResponse) => void
  ): Promise<DeleteHTTPServiceRouteResponse> {
...
接口入参中的 Tag 为文档型数据库的实例 Id，可以通过 [DescribeEnvs](https://cloud.tencent.com/document/api/876/34820) 接口返回的 EnvList[0].Databases[0].InstanceId 获取。
   ...
   */
  async RepairPGUserMigrationHistory(
    req: RepairPGUserMigrationHistoryRequest,
    cb?: (error: string, rep: RepairPGUserMigrationHistoryResponse) => void
  ): Promise<RepairPGUserMigrationHistoryResponse> {
...
销毁后可以通过 [DescribeMySQLTaskStatus](https://cloud.tencent.com/document/api/876/128183) 接口查询销毁结果，如果 `Response.Data. Status = FAILED ` 表示销毁失败，可以重新调用销毁接口重试。
   ...
   */
  async CreateHTTPServiceRoute(
    req: CreateHTTPServiceRouteRequest,
    cb?: (error: string, rep: CreateHTTPServiceRouteResponse) => void
  ): Promise<CreateHTTPServiceRouteResponse> {
...
查询云函数、云存储和数据库表的基础权限配置。支持单个资源查询和批量查询。
     */
  async DescribeResourcePermission(
    req: DescribeResourcePermissionRequest,
    cb?: (error: string, rep: DescribeResourcePermissionResponse) => void
  ): Promise<DescribeResourcePermissionResponse> {
...
返回的 KeyID 和 PrivateKey 需与环境 ID 一起组装为 JSON 配置文件，供客户端 Admin SDK 初始化时使用，文件格式如下：
{
  ...
  "env_id": "<请求时传入的 EnvId>"
     ...
     */
  async CreateCustomLoginKey(
    req: CreateCustomLoginKeyRequest,
    cb?: (error: string, rep: CreateCustomLoginKeyResponse) => void
  ): Promise<CreateCustomLoginKeyResponse> {
  ...
  async AssumeRoleForAllocatedEnv(
    req: AssumeRoleForAllocatedEnvRequest,
    cb?: (error: string, rep: AssumeRoleForAllocatedEnvResponse) => void
  ): Promise<AssumeRoleForAllocatedEnvResponse> {
   ...
   */
  async PreviewPGUserMigrations(
    req: PreviewPGUserMigrationsRequest,
    cb?: (error: string, rep: PreviewPGUserMigrationsResponse) => void
  ): Promise<PreviewPGUserMigrationsResponse> {
...
**绑定自定义 CLS 日志主题需调用腾讯云 CLS「[DescribeTopics](https://cloud.tencent.com/document/api/614/56454)」接口，按传入的 `Region` 拉取用户日志主题列表，仅筛选 `AssumerName` 为空的自有主题，并将其 `LogsetId`、`TopicId` 分别回填为绑定参数 `ClsLogsetId`、`ClsTopicId`（地域取请求参数 `Region` 作为 `ClsRegion`）。**
   ...
   */
  async AllocateEnv(
    req: AllocateEnvRequest,
    cb?: (error: string, rep: AllocateEnvResponse) => void
  ): Promise<AllocateEnvResponse> {
...
更新成功后，可通过 [DescribeAIModels](https://cloud.tencent.com/document/product/876/131318) 接口查询最新分组配置。
  ...
  async UpdateAIModel(
    ...
    cb?: (error: string, rep: UpdateAIModelResponse) => void
  ): Promise<UpdateAIModelResponse> {
...
若自定义登录（CUSTOM）或邮箱登录（EMAIL）身份源尚不存在，调用该接口时将自动创建。
   ...
   */
  async DescribeEnvPlans(
    req: DescribeEnvPlansRequest,
