...
import {
  ...
  ManagedRuleGroup,
  DummyParseZoneFullConfigResponse,
...
CNAME 模式接入时，若您未完成站点归属权校验，本接口将为您返回域名归属权验证信息，您可以单独对域名进行归属权验证，详情参考 [站点/域名归属权验证](https://cloud.tencent.com/document/product/1552/70789)。
   ...
   */
  async EdgeKVDelete(
    req: EdgeKVDeleteRequest,
    cb?: (error: string, rep: EdgeKVDeleteResponse) => void
  ): Promise<EdgeKVDeleteResponse> {
...
该功能仅企业版套餐支持，并且该功能当前仍在内测中，如需使用，请[联系我们](https://cloud.tencent.com/online-service?from=connect-us)。
   ...
   */
  async CreateEdgeKVNamespace(
    req: CreateEdgeKVNamespaceRequest,
    cb?: (error: string, rep: CreateEdgeKVNamespaceResponse) => void
  ): Promise<CreateEdgeKVNamespaceResponse> {
...
在触发[申请免费证书接口](https://cloud.tencent.com/document/product/1552/90437)后，您可以通过本接口检查免费证书申请结果。在免费证书申请成功后， 还需要通过[配置域名证书](https://cloud.tencent.com/document/product/1552/80764)接口配置，才能将免费证书部署至加速域上。
   ...
   */
  async EdgeKVList(
    req: EdgeKVListRequest,
    cb?: (error: string, rep: EdgeKVListResponse) => void
  ): Promise<EdgeKVListResponse> {
...
在 NS 接入模式下，您也可以通过本接口来查询 NS 服务器是否切换成功，详情参考 [修改 DNS 服务器](https://cloud.tencent.com/document/product/1552/90452)。
   ...
   */
  async CreateFunctionReplica(
    req: CreateFunctionReplicaRequest,
    cb?: (error: string, rep: CreateFunctionReplicaResponse) => void
  ): Promise<CreateFunctionReplicaResponse> {
   ...
   */
  async EdgeKVGet(
    req: EdgeKVGetRequest,
    cb?: (error: string, rep: EdgeKVGetResponse) => void
  ): Promise<EdgeKVGetResponse> {
   ...
   */
  async ModifyFunctionReplica(
    req: ModifyFunctionReplicaRequest,
    cb?: (error: string, rep: ModifyFunctionReplicaResponse) => void
  ): Promise<ModifyFunctionReplicaResponse> {
...
2. 本接口默认返回防护后的流量请求数据，用户可在 `Filters.mitigatedByWebSecurity` 中自定义查询已防护缓释的数据。
   ...
   */
  async ModifyEdgeKVNamespace(
    req: ModifyEdgeKVNamespaceRequest,
    cb?: (error: string, rep: ModifyEdgeKVNamespaceResponse) => void
  ): Promise<ModifyEdgeKVNamespaceResponse> {
...
> 建议您在账号下已存在套餐时调用本接口创建站点，请在入参时传入 PlanId ，直接将站点绑定至该套餐；不传入 PlanId 时，创建出来的站点会处于未激活状态，无法正常服务，您需要通过 [BindZoneToPlan](https://cloud.tencent.com/document/product/1552/83042) 完成套餐绑定之后，站点才可正常提供服务 。若您当前没有可绑定的套餐时，请前往控制台购买套餐完成站点创建。
   ...
   * 本接口用于查询边缘函数的副本列表。
   */
  async DescribeFunctionReplicas(
    req: DescribeFunctionReplicasRequest,
    cb?: (error: string, rep: DescribeFunctionReplicasResponse) => void
  ): Promise<DescribeFunctionReplicasResponse> {
...
设置环境变量后，可在函数代码中使用，具体参考 [边缘函数引入环境变量](https://cloud.tencent.com/document/product/1552/109151#0151fd9a-8b0e-407b-ae37-54553a60ded6)。
   ...
   */
  async DescribeEdgeKVNamespaces(
    req: DescribeEdgeKVNamespacesRequest,
    cb?: (error: string, rep: DescribeEdgeKVNamespacesResponse) => void
  ): Promise<DescribeEdgeKVNamespacesResponse> {
...
如果您需要配置自有证书，请先将证书上传至 [SSL证书控制台](https://console.cloud.tencent.com/certoverview)，然后在本接口中传入对应的证书 ID。详情参考 [部署自有证书至 EdgeOne 域名
   ...
   */
  async DeleteEdgeKVNamespace(
    req: DeleteEdgeKVNamespaceRequest,
    cb?: (error: string, rep: DeleteEdgeKVNamespaceResponse) => void
  ): Promise<DeleteEdgeKVNamespaceResponse> {
...
3.同时存在多个任务时将按照任务创建时间依次处理。
   ...
   */
  async ModifyFunctionComponentBindings(
    req: ModifyFunctionComponentBindingsRequest,
    cb?: (error: string, rep: ModifyFunctionComponentBindingsResponse) => void
  ): Promise<ModifyFunctionComponentBindingsResponse> {
...
该功能仅企业版套餐支持，并且该功能当前仍在内测中，如需使用，请[联系我们](https://cloud.tencent.com/online-service?from=connect-us)。
   ...
   */
  async DescribeFunctionComponentBindings(
    req: DescribeFunctionComponentBindingsRequest,
    cb?: (error: string, rep: DescribeFunctionComponentBindingsResponse) => void
  ): Promise<DescribeFunctionComponentBindingsResponse> {
   ...
   * 本接口用于删除指定的边缘函数副本。
   */
  async DeleteFunctionReplica(
    req: DeleteFunctionReplicaRequest,
    cb?: (error: string, rep: DeleteFunctionReplicaResponse) => void
  ): Promise<DeleteFunctionReplicaResponse> {
   ...
   */
  async DummyParseZoneFullConfig(
    req?: DummyParseZoneFullConfigRequest,
    cb?: (error: string, rep: DummyParseZoneFullConfigResponse) => void
  ): Promise<DummyParseZoneFullConfigResponse> {
